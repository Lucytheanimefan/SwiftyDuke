...
class SwiftyDukeTests: XCTestCase {
    ...
    func testAuth(){
        ...
        SDAuthenticator.shared.authenticate(clientID: "curriculum-mobile", clientSecret: "##iLZ%e@z@TYPGcGb%Ho9fgr$K8k79G$xCm#ut91zI=dwv=EHC", redirectURI: "http://localhost:3000/duke_oauth/callback") { (response) in
            expect.fulfill()
        }
        waitForExpectations(timeout: 100) { (error) in
            os_log("Error: %@", error.debugDescription)
